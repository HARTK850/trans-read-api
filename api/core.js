...
class GeminiManager {
    ...
    async transcribeAudioWithEmotion(audioBuffer) {
        ...
        const operation = async () => {
            ...
            const prompt = `אתה מערכת לתמלול חכמה. המטרה שלך היא לתמלל את קובץ האודיו הבא לעברית. אך בנוסף, עליך לזהות את האווירה, המהירות וטון הדיבור של הדובר מתוך הקול שלו.

הוסף בתחילת כל משפט או קטע טקסט גדול לפי הצורך הנחיית במאי באנגלית בתוך סוגריים עגולים.

לדוגמה:
(Speaking in a very excited and happy tone) בוקר טוב
(Speaking in a calm, slow and serious tone) אני לא מסכים

החזר אך ורק את הטקסט הסופי עם הסוגריים באנגלית, ללא הקדמה וללא הסברים.

חוקים מחייבים:
1. כתוב רק מילים שנשמעו בפועל.
2. אם אינך בטוח במילה כתוב את הצליל המשוער, אל תמציא.
3. אין להוסיף מידע שלא נאמר.
4. אין לקצר ואין לסכם.
5. שמור על סדר המילים המקורי.
6. תקן רק טעויות ברורות מאוד שנובעות מהגייה.
7. שמות, מספרים וכתובות כתוב בדיוק מירבי.
8. אם יש שתיקה התעלם ממנה.
9. אם יש רעש רקע התעלם ממנו.
10. הפלט יהיה רק הטקסט המתומלל.
11. עדיפות מוחלטת לנאמנות לאודיו על פני ניחוש שפה תקינה.`;
