...
// ניקוי טקסט מסימנים אסורים בימות המשיח (נקודה, מקף, סוגריים וכו')
...
module.exports = async function handler(req, res) {
    ...
    // עצירת פעולה אם מדובר בדיווח על ניתוק מצידו של הלקוח
    if (query.hangup === "yes") {
        console.log(`[Hangup] שיחה נותקה על ידי הלקוח: ${query.ApiCallId}`);
    ...
    try {
        ...
        switch (state) {
            case 0:
                // שלב 0: הקלטה. 
                // שימו לב שפרמטר 6 הוא "no" כדי לדלג על תפריט האישור של ימות ("לאישור הקישו 1...").
                yemotRes = buildYemotResponse(
                    "read", 
                    `t-ברוכים הבאים למערכת היצירה הקולית הקליטו את הטקסט שברצונכם להקריא ולאחר מכן הקישו סולמית=UserAudioRecord,no,record,${TEMP_FOLDER},${ApiCallId}_main,no,yes,no,2,120`,
            ...
            case 1:
                ...
                // שרשור הודעות תקין לימות המשיח עם נקודה מחוץ לטקסט
                yemotRes = buildYemotResponse(
                    "read",
